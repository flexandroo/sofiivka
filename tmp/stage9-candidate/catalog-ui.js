(function () {
  ...
  if (!catalog || window.sofievkaCatalogUI) return;

  const state = { expandedGroups: new Set(), brandQuery: "", visibleCount: 24 };
  const escapeHtml = value => String(value ?? "").replace(/[&<>'"]/g, character => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" })[character]);
  const money = value => catalog.formatPrice(value);
  ...
  function renderProductCard(product, options = {}) {
    ...
    const inStock = availability.purchasable;
  ...
  const trackProductImages = root => {
    ...
    images.forEach(image => {
      ...
      image.addEventListener("load", () => {
        image.hidden = false;
        image.closest(".product-card__image, .search-suggestion__image, .compare-product-image, .cart-item__image")?.classList.remove("is-fallback");
      });
  ...
  function render({ pageName = "catalog" } = {}) {
    ...
    if (ctx.isBrand && !total) return `${introMarkup(ctx, 0)}<section class="catalog-workspace catalog-workspace--state"><div class="container"><div class="catalog-state"><p class="page-kicker">Асортимент готується</p><h2>Товарів цього бренду поки немає</h2><p>Бренд залишається в офіційному реєстрі. Перейдіть до всього каталогу, щоб переглянути доступні товари.</p><a class="button button--secondary" href="/catalog">Увесь каталог</a></div></div></section>`;
  ...
  function labelFor(key, value) {
    ...
    if (key === "availability") return catalog.availabilityState(value).label;
  ...
  function bindSearch(form = document.querySelector("[data-search]"), options = {}) {
    ...
    const option = ({ href, title, meta, image = "", kind, index }) => `<a id="${escapeHtml(instanceId)}-option-${index}" class="search-suggestion search-suggestion--${kind}" href="${escapeHtml(href)}" role="option" aria-selected="false">${image ? `<span class="search-suggestion__image"><img src="${escapeHtml(image)}" width="56" height="56" alt="" loading="lazy" decoding="async" data-product-image><span aria-hidden="true">Фото</span></span>` : `<span class="search-suggestion__mark" aria-hidden="true">${kind === "brand" ? "B" : kind === "category" ? "C" : "S"}</span>`}<span class="search-suggestion__text"><strong>${escapeHtml(title)}</strong><small>${escapeHtml(meta)}</small></span><span class="search-suggestion__arrow" aria-hidden="true">→</span></a>`;
  ...
  function homeCards() {
    ...
    return catalog.activeSections.map(section => {
      ...
      return `<a class="category-card category-card--${escapeHtml(section.id)}" href="${catalog.sectionUrl(section.id)}"><img src="${imageBySection[section.id]}" width="1536" height="1024" loading="lazy" alt="${escapeHtml(section.name)}"><span class="category-card__content"><span class="category-card__title"><strong>${escapeHtml(section.name)}</strong><em>${catalog.productsForSection(section.id).length}</em></span><small>${children.length ? children.map(category => category.name).join(" · ") : section.description}</small></span></a>`;
    }).join("");
  }
