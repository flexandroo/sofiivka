...
(function () {
  ...
  const suppliers = Object.freeze({
    ecosoft: Object.freeze({ id: "ecosoft", brandId: "ecosoft", sourceName: "Sofiivka Water / Ecosoft" }),
...
(function () {
  ...
  function enrichEcosoft(product, mapping) {
    ...
    const description = readableEcosoftDescription(product, mapping.categoryId, category.title);
    ...
    return {
      ...
      officialSourceUrl: `https://ecosoft.ua/ua/search/?search_api_fulltext=${encodeURIComponent(product.sku)}`,
      officialCatalogEdition: "Ecosoft 2026",
...
(function () {
  ...
  const availabilityPresentation = Object.freeze({
    in_stock: Object.freeze({ status: "in_stock", label: "В наявності", modifier: "available", purchasable: true }),
    out_of_stock: Object.freeze({ status: "out_of_stock", label: "Немає в наявності", modifier: "unavailable", purchasable: false }),
    pre_order: Object.freeze({ status: "pre_order", label: "Під замовлення", modifier: "pending", purchasable: false }),
    unknown: Object.freeze({ status: "unknown", label: "Наявність уточнюйте", modifier: "pending", purchasable: false })
  });

  function formatPrice(value) {
    const amount = Number(value);
    return Number.isFinite(amount) && amount > 0 ? `${priceFormatter.format(amount)} грн` : "";
  }

  function availabilityState(value) {
    const status = String(value || "unknown").toLocaleLowerCase("en");
    return availabilityPresentation[status] || availabilityPresentation.unknown;
  }
...
(function () {
  ...
  function specificationGroups(product) {
    ...
    if (normalizedIds.length <= 5) {
      ...
      return items.length ? Object.freeze([Object.freeze({ id: "technical", title: "Технічні дані", items: Object.freeze(items) })]) : Object.freeze([]);
    }
  ...
  function images(product) {
    const candidates = [product.image, ...(Array.isArray(product.images) ? product.images : []), ...(Array.isArray(product.gallery) ? product.gallery : [])]
      ...
      .filter(Boolean);
    return Object.freeze([...new Set(candidates)].slice(0, 8));
  }
  ...
  function purchase(product) {
    ...
    const inventory = catalog.availabilityState(status);
    ...
    return Object.freeze({
      ...
      statusLabel: inventory.label,
      amount: Number.isFinite(amount) && amount > 0 ? amount : null,
      oldAmount: Number.isFinite(oldAmount) && oldAmount > amount ? oldAmount : null,
      purchasable: inventory.purchasable && Number.isFinite(amount) && amount > 0
    });
  }
