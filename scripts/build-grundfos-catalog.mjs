...
const SOURCE_FILE = path.join(ROOT, "tmp", "grundfos-source", "products.json");
...
function keyFeatures(details, classification) {
  const patterns = classification.category === "heating"
...
function editorialDescription(record, classification, details, features) {
  ...
  const constructionRows = details.filter(([label]) => /тип електродвигуна|з електродвигуном|тепловий захист|захист електродвигуна|реле рівня|автоматичний нічний|сітчастий фільтр|кабельна вилка|роз'єм електроживлення|тип штекера|тип робочого колеса|монтаж$/i.test(label)).slice(0, 6);
  ...
  const sections = [
    ...
    { title: "Призначення", paragraphs: [uses.length ? `Офіційно заявлені сфери застосування: ${uses.map(lowerFirst).join("; ")}.` : `${classification.type} призначений для роботи у відповідній побутовій інженерній системі.`] },
  ...
  sections.push({ title: "Підбір", paragraphs: ["Під час підбору потрібно звірити робочу точку системи, параметри електроживлення, тип приєднання, допустиму температуру середовища та умови монтажу з технічними даними цього виконання."] });
  const shortBase = `Grundfos ${model} — ${purpose}`;
  const shortDescription = `${shortBase.length <= 218 ? shortBase : `Насос Grundfos ${model} серії ${classification.series}`}.`;
...
async function main() {
  ...
  const products = await mapLimit(records, 10, async record => {
    ...
    const docs = selectDocuments(documentGroups[groupKey(record)]);
    ...
    const title = `Насос Grundfos ${clean(record.product?.name)}`;
    ...
    return {
      ...
      seriesId: `grundfos-${slugify(classification.series)}`,
      model: clean(record.product?.name),
      title,
      productName: title,
      product_name: title,
