...
assert.equal(baxiProducts.filter(product => product.images.length >= 1).length, 76, "BAXI confirmed product-photo coverage changed");
assert.equal(baxiProducts.filter(product => product.images.length === 0).length, 17, "BAXI overview pages without a confirmed product photo changed");
assert.ok(baxiProducts.every(product => product.images.every(image => image.startsWith("/assets/products/baxi/"))), "BAXI images must be local");
assert.equal(baxiProducts.reduce((total, product) => total + product.images.length, 0), 77, "BAXI product image coverage changed");
assert.equal(new Set(baxiProducts.flatMap(product => product.images)).size, 37, "BAXI unique product image coverage changed");
assert.ok(baxiProducts.every(product => product.imageSources.every(image => /^https:\/\/baxi\.ua\/assets\/uploads\/images\/image_boiler\/e-catalog\//.test(image.source))), "BAXI galleries must contain official product packshots only");
...
assert.equal(buderusProducts.reduce((total, product) => total + product.images.length, 0), 45, "Buderus gallery image coverage changed");
assert.ok(buderusProducts.every(product => product.imageSources.every(image => !/(?:award|diplom|certificate|badge|medal)/i.test(image.source))), "Buderus galleries must exclude award and certificate artwork");
