...
(async () => {
  ...
  async function inspect() {
    while (cursor < urls.length) {
      ...
      try {
        ...
        if (!response.ok || !/pdf|octet-stream/i.test(response.headers.get("content-type") || "")) {
  ...
  assert.deepEqual(failed, [], `unavailable BAXI/Buderus documents: ${JSON.stringify(failed)}`);
